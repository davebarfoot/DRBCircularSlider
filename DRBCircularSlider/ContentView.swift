...
struct ContentView : View {
    ...
    var body: some View {
        VStack {
            ZStack {
                DRBCircularSlider(size: 100.0, stroke: 5.0, initial: 45.0, value: $Slidervalue)
                Text(String(format: "%.2f",self.Slidervalue))
                
            }
            ZStack {
                DRBCircularSlider(size: 200.0, stroke: 10.0, initial: 72.0, value: $Slidervalue2)
                Text(String(format: "%.2f",self.Slidervalue2))
                
            }
            ZStack {
                DRBCircularSlider(size: 300.0, stroke: 20.0, initial: 135.0, value: $Slidervalue3)
